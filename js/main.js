...
// --- ИНИЦИАЛИЗАЦИЯ РАМКИ (БАНКИ) ---
const frameRenderer = new FrameRenderer(CONFIG);
...
function drawCell(e) {
  ...
  // Получаем координаты ИГРОВОГО холста
  const rect = renderer.canvas.getBoundingClientRect();

  // Масштабируем координаты мыши в соответствии с физическим разрешением сетки
  const scaleX = renderer.canvas.width / rect.width;
  const scaleY = renderer.canvas.height / rect.height;

  // ВАЖНО: В предыдущем запросе я настроил CSS так, что gameCanvas отступает от краев банки.
  // getBoundingClientRect() вернет правильные координаты именно внутренней части (самой игры),
  // поэтому курсор не будет промахиваться.
  ...
  for (let i = -1; i <= 1; i++) {
    for (let j = -1; j <= 1; j++) {
      ...
      // Проверка на выход за границы сетки предотвратит ошибки
...
musicVolumeSlider.addEventListener(
  "input",
  (e) => (bgMusic.volume = e.target.value),
);
